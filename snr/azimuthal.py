...
def local_peak(data, window=5):
    ...
    index = []
    contrast = []
    truth = [ True for i in range(win_half) ] + [ False for i in range(win_half) ]
        ...
        if not np.isnan(dd).any():
            ...
            if len(test) == window - 1:
                if (test == truth).all():
                    ...
                    # contrast.append( float(max(dd)-min(dd)) / min(dd))
                    contrast.append( float(max(dd)-min(dd)) / dd.mean())
    return index, contrast
...
def radial_profile_simple(image, center, r0=60):
    ...
    for rr in range(minr, maxr):
        s = cropped[np.logical_and(rad >= rr,  rad <= rr+2)]     # ring width 2
        prof.append(s[s>0].mean())
    prof = np.array(prof)

    try:
        r_peak, v_peak, _  = best_local_peak(prof, window=3)
        r_peak = r_peak + minr + 1
    except TypeError as e:
        message = e.message
        message += ", ring: %d %d %d" % (center[0], center[1], r0)
        sys.stderr.write(message + '\n')
        r_peak, v_peak = 0, 0
    # return r_peak, v_peak
    return prof, r_peak, v_peak
